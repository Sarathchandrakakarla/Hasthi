...
def manage_places(request):
    ...
    if sess[0]:
        username = sess[1]
        if var_check(request,"View"):
            print(1)
            type = request.POST['Place_Type']
            places_list = places.objects.filter(Type = type)
            print(type)
            return render(request, 'manage_places.html', {'Username': username,'Places':places_list,'Place_Type':type})
        return render(request, 'manage_places.html', {'Username': username})
    else:
        return HttpResponse("<script>alert('Session Ended!Please Login Again!');location.replace('/auth/login_page/')</script>")
