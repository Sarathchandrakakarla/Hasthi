...
def create_checkout_session(amount,product):
  session = stripe.checkout.Session.create(
    ...
    payment_method_configuration= 'pmc_1OFI5GSFec7g44XJ7W0bgDnw',
...
def user_dashboard(request):
    ...
    if sess[0]:
        ...
        college_list = places.objects.filter(Type = 'College')
        city_list = places.objects.filter(Type = 'City')
        hours = []
        ...
        try:
            if request.GET['Action'] == "Check_Place":
                place = request.GET['Place']
                type = request.GET['Type']
                if  places.objects.filter(Name = place,Type = type).exists():
                    return HttpResponse("True")
                else:
                    return HttpResponse("False")
        except:
            pass
        return render(request, 'user_dashboard.html', {'Username': username, 'Colleges': college_list,'Cities':city_list,'Hours':hours,'Minutes':minutes})
    else:
    ...
    if sess[0]:
        ...
        if var_check(request, "Journey_Type"):
            ...
            if (journey_type == "Pending"):
                ...
                journey_list = bookings.objects.filter(
                    Username=username, Admin_Accept_Status=1,Journey_Status = 'Ongoing',Bus_No=None)
            elif (journey_type in ['Rejected','Cancelled','Completed']):
                journey_list = bookings.objects.filter(
                    Username=username, Journey_Status=journey_type)
            elif (journey_type == "Confirmed"):
                journey_list = bookings.objects.raw(f"SELECT * FROM `bookings` WHERE Username='{username}' AND Admin_Accept_Status=1 AND Pay_Status=1 AND Journey_Status='Ongoing' AND Bus_No is NOT NULL")
            user_details = user.objects.filter(Username=username)
            for u in user_details:
                u.Full_Name=",".join(u.Full_Name.split(" "))
            for i in journey_list:
                i.Date = format_date(i.Date)
            if (journey_type in ["Confirmed","Accepted"]):
                return render(request, 'journeys.html', {'Username': username, 'Journey_List': journey_list, 'Journey_Type': journey_type,'User_Details':user_details})
            else:
    ...
    if sess[0]:
        ...
        if var_check(request,'Pay'):
            ...
            for order in order_details:
                redirecturl = create_checkout_session(amount,product)
